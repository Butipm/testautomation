...
Feature: Create Subscription using API
    ...
    * eval
	...
	# Until Subscription is approved and all deployments for the first environment are created
		And retry until response.status == 'APPROVED' && response.subscriptionItems[0].channelSpecification.deployments.length == 1 && response.subscriptionItems[0].channelSpecification.targetEmitterSpecifications[0].deployments.length == 1 && response.subscriptionItems[0].dataTransportSpecification.deployments.length == 0
		And header Authorization = 'Bearer ' + accessToken
		When method GET
		Then status 200
    ...
    """
	 for(var i=0; i < response.subscriptionItems.length ; i++) {
        ...
        response.subscriptionItems[i].dataTransportSpecification.deployments.length == 0
	 }
    """
	  * def subId = response._id
	  * path '/subscriptions/'+subId
    ...
    """
	  ...
	  * def subId = response._id
	  * path '/subscriptions/'+subId
	  And retry until for(var i=0;i<response.subscriptionItems.length;i++) response.subscriptionItems[0].dataTransportSpecification.deployments.length == 0
	  And header Authorization = 'Bearer ' + accessToken
	  When method GET
	  Then status 200
