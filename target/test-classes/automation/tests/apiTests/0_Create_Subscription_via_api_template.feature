...
Feature: Create Subscription using API
	...
	Background:
		* url testsApi
		* configure ssl = true
	  	* configure retry = { count: 100, interval: 3000 }
	...
	# Until Subscription is approved and all deployments for the first environment are created
		And retry until response.status == 'APPROVED' && response.subscriptionItems[0].channelSpecification.deployments.length == 1 &&response.subscriptionItems[1].channelSpecification.deployments.length == 1 && response.subscriptionItems[0].channelSpecification.targetEmitterSpecifications[0].deployments.length == 1 && response.subscriptionItems[1].channelSpecification.targetEmitterSpecifications[0].deployments.length == 1 && response.subscriptionItems[0].dataTransportSpecification.deployments.length == 1 && response.subscriptionItems[1].dataTransportSpecification.deployments.length == 1
		And header Authorization = 'Bearer ' + accessToken
		When method GET
		Then status 200
    ...
    """
	  * def subId = response._id
	  * path '/subscriptions/'+subId
	  And retry until for(var i=0;i<response.subscriptionItems.length;i++) response.subscriptionItems[0].channelSpecification.deployments[0].status == 'IN_PROGRESS' && response.subscriptionItems[1].channelSpecification.deployments[0].status == 'IN_PROGRESS'
	  And header Authorization = 'Bearer ' + accessToken
	  When method GET
	  Then status 200
    ...
    """
	  ...
	  * def subId = response._id
	  * path '/subscriptions/'+subId
	  And retry until for(var i=0;i<response.subscriptionItems.length;i++) response.subscriptionItems[0].channelSpecification.targetEmitterSpecifications[0].deployments[0].status == 'IN_PROGRESS' && response.subscriptionItems[1].channelSpecification.targetEmitterSpecifications[0].deployments[0].status == 'IN_PROGRESS'
	  And header Authorization = 'Bearer ' + accessToken
	  When method GET
	  Then status 200
    ...
    """
	  ...
	  * def subId = response._id
	  * path '/subscriptions/'+subId
	  And retry until for(var i=0;i<response.subscriptionItems.length;i++) response.subscriptionItems[0].dataTransportSpecification.deployments[0].status == 'IN_PROGRESS' && response.subscriptionItems[1].dataTransportSpecification.deployments[0].status == 'IN_PROGRESS'
	  And header Authorization = 'Bearer ' + accessToken
	  When method GET
	  Then status 200
